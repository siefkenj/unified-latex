...
export function getOptionalArg(
    nodes: Node[],
    pos: number
): { arg: Argument; nodesRemoved: number } {
    const { argument, nodesRemoved } = gobbleSingleArgument(
        nodes,
        parseArgspec("o")[0],
        pos
    );
...
export function getGroup(
    ...
    return {
        arg: arg([], { openMark: "", closeMark: "" }),
        nodesRemoved: 0,
    };
}

export function getDelimGroup(
    nodes: Node[],
    pos: number
): { arg: Argument; nodesRemoved: number } {
    const nextArgNode = nodes[pos];
    if (match.string(nextArgNode) && nextArgNode.content.length === 1) {
        ...
        const closePos = nodes.findIndex(
            (node, i) => i > pos && match.string(node, brace)
        );
        if (closePos > pos) {
            const verbatim = arg(nodes.slice(pos + 1, closePos), {
                openMark: brace,
                closeMark: brace,
            });
